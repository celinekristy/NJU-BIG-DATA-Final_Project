...
public class PageRankBasic
{
    public static void main(String[] args)
    {
        if(args.length < 2)
        {
            System.err.println("Usage: PageRankBasic <input_edges> <output_path> [iterations]");
            System.exit(1);
        }
        ...
        int iterations; // 设置 PageRank 要迭代多少轮，默认为10轮
        if(args.length >= 3)
        {
        ...
        final double d = 0.85; // damping factor (阻尼系数)

        SparkConf conf = new SparkConf().setAppName("PageRankBasic"); // 配置环境
        JavaSparkContext sc = new JavaSparkContext(conf);

        // ------------------------- 读取边文件 clean_edges.tsv -------------------------

        JavaRDD<String> lines = sc.textFile(inputpath); // 读取每一行数据

        JavaPairRDD<String, String> edges = lines
                ...
                .filter(edge -> !edge._1.equals(edge._2)) // 过滤自己发邮件给自己的这种情况
                .distinct() // 去重
                .cache(); // 缓存这个RDD
        
        // -------------------------- 得到所有节点：sender ∪ receiver ----------------------

        JavaRDD<String> nodes = edges.keys() // 得到所有的发送人，以及得到所有的收件人，然后去重得到所有的人（节点）
            .union(edges.values())
            .distinct()
            .cache();
        ...
        JavaPairRDD<String, Iterable<String>> links = edges
            .groupByKey()
            .cache(); 
        ...
        // 顺便验证：所有 PR 之和应该 ≈ 1.0（dangling 处理对的话）
        ...
        System.out.println("Task1 finished. Output path: " + outputpath);
        sc.stop();   // 关闭 Spark
    }
}